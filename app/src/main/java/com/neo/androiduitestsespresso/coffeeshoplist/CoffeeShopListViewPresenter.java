...
import static com.neo.androiduitestsespresso.coffeeshoplist.CoffeeShopListActivity.COFFEE_SHOPS_BUNDLE_KEY;
...
class CoffeeShopListViewPresenter {
    ...
    private static final String TOKEN = "CONSUMER_TOKEN";
    ...
    @Nullable
    // used to notify idling Resource when search for a coffee shopList is complete
    ...
    CoffeeShopsIdlingResource getCoffeeShopsIdlingResource() {
        if (null == coffeeShopsIdlingResource) {
            ...
            if (coffeeShopListModel.getCoffeeShops() != null && !coffeeShopListModel.getCoffeeShops().isEmpty()) {
                // if statement checks that the list is not empty, then notify the idling resource that search's completed
